...
[cite_start]Ce dépôt contient le code d'Infrastructure as Code (IaC) permettant de déployer la plateforme de streaming "StreamFlex"[cite: 3]. [cite_start]L'architecture est conçue pour héberger deux microservices conteneurisés de manière hautement disponible, sécurisée, et résiliente[cite: 3].

[cite_start]L'infrastructure est pensée pour être **éphémère** : elle peut être déployée en quelques minutes pour des démonstrations ou des tests de reprise sur sinistre (Disaster Recovery) entre les régions `us-east-1` et `us-west-2`[cite: 32], puis détruite intégralement pour optimiser les coûts.
...
* [cite_start]**Le choix :** Amazon Elastic Container Service (ECS) avec le type de lancement AWS Fargate[cite: 19].
...
* **L'argumentaire :** Fargate est une solution 100 % Serverless. [cite_start]Il permet de définir les ressources au strict minimum requis (0.25 vCPU et 512 Mo de RAM par tâche)[cite: 21], garantissant une empreinte budgétaire minime tout en déléguant la gestion de la flotte de serveurs à AWS.

### 3. Réseau & Routage : Application Load Balancer (ALB)
* [cite_start]**Le choix :** Un ALB public unique couplé à des règles de routage basées sur le chemin (Path-based routing)[cite: 24, 25].
...
* **L'argumentaire :** Le catalogue de vidéos est un cas d'usage parfait pour le NoSQL (requêtes rapides et prévisibles). DynamoDB offre un mode de facturation "à la demande" (On-Demand) totalement gratuit lorsque l'API n'est pas sollicitée. [cite_start]RDS permet de conserver l'intégrité relationnelle pour les profils utilisateurs, tout en respectant les consignes de sécurité (Enhanced Monitoring désactivé)[cite: 28].
...
* [cite_start]**L'argumentaire :** Utiliser un registre public garantit que lors du test de basculement d'urgence (Crash-test Région) [cite: 41][cite_start], le cluster ECS démarré en `us-west-2` pourra puller les images de conteneurs instantanément sans rencontrer d'erreurs "Access Denied" liées aux rôles d'exécution Fargate[cite: 32].
